...
nv.models.indentedTree = function() {
  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      var textVal = series.select('text').data();
      ...
      series.select('circle')
          .style('fill', function(d,i) { return d.color || color(d,i)})
          .style('stroke', function(d,i) { return d.color || color(d, i) });
      //console.log(series.select('text'));
      //console.log(series.select('text').data());
      series.select('text').text(getKey);
      series.select('text').each(insertLinebreaks);
            ...
            .attr('transform', function(d, i) {
              var gap = 20;
               if(typeof(d.key) != 'undefined'){
                  ...
                  if(d.key.length > 9 && keySplit.length > 1){
                    prevCount = 1;
                  }else{
                    prevCount = 0;
                  }
              ...
              return 'translate(' + xPositions[i % seriesPerRow] + ',' + (5 + Math.floor(i / seriesPerRow) * gap) + ')';
            });
...
nv.models.pie = function() {
  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      if(donut) divisor = 2;
      wrap.attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
      g.select('.nv-pie').attr('transform', 'translate(' + availableWidth / divisor + ',' + availableHeight / 2 + ')');
  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      if (!data || !data.length) {
        ...
        noDataText
          .attr('x', margin.left + availableWidth / 1.6)
          .attr('y', margin.top + availableHeight / 2)
          .text(function(d) { return d });
      ...
      var leftMargin = width - margin.left;

        wrap.select('.nv-legendWrap')
            .attr('transform', 'translate('+(leftMargin - 110)+',' + 10 +')');
      }
